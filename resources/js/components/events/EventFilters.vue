    ...
    <div class="rounded-xl border bg-card p-4 shadow-sm">
        <div class="flex flex-wrap items-end gap-3">
            <div class="flex min-w-44 flex-1 flex-col gap-1">
                ...
                <select id="city" v-model="filters.city" :class="selectClass" @change="emit('change')">
                    <option :value="null">All locations</option>
                    <option v-for="city in cities" :key="city.label" :value="city.label">{{ city.label }}</option>
                </select>
            </div>

            <div class="flex flex-col gap-1">
                <label class="text-xs font-medium text-muted-foreground" for="from">From</label>
                <input id="from" v-model="filters.from" type="date" :class="selectClass" @change="emit('change')" />
            </div>

            <div class="flex flex-col gap-1">
                <label class="text-xs font-medium text-muted-foreground" for="to">To</label>
                <input id="to" v-model="filters.to" type="date" :class="selectClass" @change="emit('change')" />
            </div>

            <div class="flex flex-col gap-1">
                <label class="text-xs font-medium text-muted-foreground" for="type">Category</label>
                <select id="type" v-model="filters.type" :class="selectClass" @change="emit('change')">
            ...
            <div class="flex flex-col gap-1">
                ...
                <select id="status" v-model="filters.status" :class="selectClass" @change="emit('change')">
                    <option value="all">All statuses</option>
                    <option v-for="status in statuses" :key="status" :value="status">{{ status.replace('_', ' ') }}</option>
                </select>
            </div>

            <Button variant="ghost" size="sm" class="h-9" @click="emit('reset')">
